  ...
  return (
    <Form method={method} className={classes.form}>
      ...
      <div className={classes.actions}>
        ...
        <button disabled={isSubmitted} >{isSubmitted ? "Submitting.." : "Submit"}</button>
      </div>
    </Form>
  );
}
...
export async function eventSubmitAction ({ request, params }) {
  ...
  if(response.status === 422){
      console.log("inside response status 422")
      return response;
  }
  if (!response.ok) {
      throw json(
